...
class JsonWebSignatureSpec extends FunSpec with Matchers {
  ...
  describe("JsonWebSignature") {
    val secretKey = Array[Byte](115, 101, 99, 114, 101, 116, 75, 101, 121)
    ...
    it("Should be able to sign a JWT with a key that is not a valid UTF string") {
      val key = DatatypeConverter.parseHexBinary("fefeffff")
      JsonWebSignature(HS256, "someData", key) should equal(Array[Byte](46, -61, -117, 6, 116, -17, 15, -50, 52, 4, -114, 27, 116, 20, 14, -8, -119, -1, -110, 55, -4, 86, -25, 21, -88, 113, 58, 98, 123, 75, -37, 88))
    }
  }
